 ...
 */ function searchEmployees(searchTerm) {
    ...
    const filtered = successStories.filter((person)=>person.name.toLowerCase().includes(searchTerm.toLowerCase()) || person.company.toLowerCase().includes(searchTerm.toLowerCase()) || person.role.toLowerCase().includes(searchTerm.toLowerCase()));
 ...
 */ function init() {
    try {
        ...
        // Show error state
        const grid = document.getElementById('employeeGrid');
        if (grid) grid.innerHTML = '<div class="error">Failed to load employees</div>';
    }
}
...
//# sourceMappingURL=placement.93460a30.js.map
