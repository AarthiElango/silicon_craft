...
class TestimonialsCarousel {
    ...
    updateCardWidth() {
        ...
        if (screenWidth <= 480) {
            this.cardWidth = 288; // 18rem
        } else if (screenWidth <= 768) {
            this.cardWidth = 320; // 20rem
        } else {
            this.cardWidth = 384; // 24rem
        }
    }
    ...
    animate() {
        ...
        if (Math.abs(this.translateX) >= maxTranslate) {
            this.translateX = 0; // Seamless reset
        } else {
            this.translateX -= this.speed; // Smooth pixel-by-pixel movement
        }
    ...
    handleMouseEnter() {
        ...
        if (this.pauseIndicator) {
            this.pauseIndicator.textContent = 'Paused - move cursor away to resume';
        }
    }
    ...
    handleMouseLeave() {
        this.isAutoPlaying = true;
        if (this.pauseIndicator) {
            this.pauseIndicator.textContent = 'Hover to pause';
        }
    }
...
const TestimonialUtils = {
    ...
    generateStars(rating) {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= rating) {
                stars += `<svg class="star star-filled" viewBox="0 0 24 24" fill="currentColor"><path d="${starPath}"/></svg>`;
            } else {
                stars += `<svg class="star star-empty" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="${starPath}"/></svg>`;
            }
        }
        return stars;
    },
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { TestimonialsCarousel, TestimonialUtils };
}
